...
fn a_writer_inside_a_subshell_gets_the_anchored_base() {
    ...
    let (v, r) = common::decision_at(&cfg(), "cd /tmp/proj && ( echo x > f.txt )", "/somewhere/else");
    assert_eq!(v, "allow", "{r}");
...
fn nested_scoped_movers_compose_inside_their_scope() {
    ...
    let (v, r) = common::decision_at(&cfg(), "cd /etc && ( cd /tmp/proj && echo x > f.txt )", "/x");
    assert_eq!(v, "allow", "{r}");
}
...
fn positions_before_a_same_process_body_mover_resolve() {
    ...
    let (v, r) =
        common::decision_at(&cfg(), "echo x > f.txt; while true; do cd /etc; done", "/tmp/proj");
    assert_eq!(v, "allow", "{r}");
}
...
fn positions_after_a_same_process_body_mover_ask() {
    ...
    let (v, r) = common::decision_at(&cfg(), "{ cd /etc; }; echo y > g.txt", "/tmp/proj");
    assert_eq!(v, "ask", "{r}");
    let (v2, r2) = common::decision_at(&cfg(), "echo x > f.txt; { cd /etc; }; true", "/tmp/proj");
...
fn a_condition_list_mover_contaminates_the_body_it_gates() {
    ...
    let (v, r) =
        common::decision_at(&cfg(), "while cd /etc; do echo x > rel.txt; done", "/tmp/proj");
    assert_eq!(v, "ask", "{r}");
}
...
fn a_redirect_after_a_subshell_binds_to_its_own_command_not_the_bodys() {
    ...
    let (v, r) = common::decision_at(
        &cfg(),
        "( cd /etc; echo x > /tmp/proj/inner.txt ); echo y > outer.txt",
        "/tmp/proj",
    );
    assert_eq!(v, "allow", "{r}");
}
...
fn a_wrapper_inside_a_body_parents_onto_the_bodys_scope() {
    ...
    let (v, r) = common::decision_at(&cfg(), "cd /tmp/proj && ( bash -c 'echo x > f.txt' )", "/x");
    assert_eq!(v, "allow", "{r}");
}
...
fn an_unowned_redirect_resolves_in_its_own_scope_not_scope_zero() {
    ...
    for cmd in [
        "( cd /etc; { echo hi; } > rel.txt )",
        "( cd /etc; for f in 1; do echo hi; done > rel.txt )",
        "( cd /etc; true || echo hi > rel.txt )",
    ] {
        let (v, r) = common::decision_at(&cfg(), cmd, "/tmp/proj");
    ...
    let (v, r) = common::decision_at(&cfg(), "( cd /etc ) ; { echo hi; } > rel.txt", "/tmp/proj");
    assert_eq!(v, "allow", "{r}");
    let (v2, r2) = common::decision_at(&cfg(), "( { echo hi; } > rel.txt )", "/tmp/proj");
    assert_eq!(v2, "allow", "{r2}");
}
...
fn a_synthesized_wrapper_mover_still_poisons_its_scope() {
    ...
    let (v, r) =
        common::decision_at(&cfg(), "find . -exec cd {} \\; ; echo x > f.txt", "/tmp/proj");
    assert_eq!(v, "ask", "{r}");
}
